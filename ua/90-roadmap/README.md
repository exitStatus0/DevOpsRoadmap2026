...
#### Моніторинг та Observability (2-3 тижні)
```
Observability-план:
...
│   ├── Service monitors
│   ├── PromQL базово
│   └── Dashboards та alerts
├── ELK або Loki для логів
│   ├── Збір логів з K8s
│   ├── Фільтрація та пошук
│   └── Log-based алерти
├── Alertmanager
│   ├── Routing правила
│   ├── Інтеграція зі Slack/PagerDuty
│   └── Silence та inhibit
└── SLI/SLO/SLA
    ├── Що це і навіщо
    ├── Визначення SLO для сервісу
    └── Error budget
```
...
| Kubernetes | 20% | Найскладніший та найцінніший навик |
| Cloud (AWS) | 20% | Основа всієї інфраструктури |
| IaC (Terraform) | 15% | Обов'язковий для будь-якої серйозної позиції |
...
Зарплата: +20-35% до базового DevOps
```
...
Зарплата: +15-30% до базового DevOps
```
...
Зарплата: +25-40% до базового DevOps
```
...
Демонструє: Усі 5 факторів. Це ваш «головний» проєкт.
```

### Проєкт 2: «IaC Library» (2-3 тижні)
...
### Проєкт 3: «Security Pipeline» (2-3 тижні)
...
### Проєкт 4: «MLOps Pipeline» (3-4 тижні)
...
### Проєкт 5: «Incident Response System» (1-2 тижні)

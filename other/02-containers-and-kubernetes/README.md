...
- **Минимальный путь:** Сначала Docker. Не уходите в Kubernetes, пока не умеете локально собирать, запускать, инспектировать и дебажить контейнеры.
- **Порог найма:** Выйдите на уровень **уверенный** с Deployments, Services, Ingress, Helm, probes, resource limits и системным troubleshooting.
- **Оставьте на потом:** Операторы, service mesh, multi-cluster и глубокую настройку Gateway API или Karpenter.

---

## Почему это важно в 2026

Docker -- это базовый навык. Kubernetes -- частый стандарт оркестрации и важный шаг для cloud-native ролей.
...
В 2026 году:
- **Docker** -- ожидается на многих командах как базовый минимум
- **Kubernetes** -- часто отличает начальный уровень от более уверенного mid-level
- **Container security** -- сканирование образов, non-root и policy checks всё чаще становятся нормой (см. [Фактор 3: DevSecOps](../03-devsecops/))
- **GitOps** -- деплой через Git (ArgoCD, Flux) всё чаще встречается в зрелых командах
